...
locals {
  configurations = {
    ...
    basename(f.path) => yamldecode(file(f))
  }
}
...
module "app" {
  ...
  vpc_id = each.values.beanstalk.networking.vpc_id

  server_types = each.value.beanstalk.instance.server_types


  beanstalk_application          = each.value.beanstalk.application
  beanstalk_ec2_key              = each.value.beanstalkinstance.ec2_key
  beanstalk_ami_id               = each.value.beanstalk.instance.ami_id
  beanstalk_instance_port        = each.value.beanstalk.instance_port
  beanstalk_enable_spot          = each.value.beanstalk.enable_spot
  beanstalk_default_retention    = each.value.beanstalk.default_retention
  ...
  beanstalk_instance_profile     = each.value.beanstalk.iam.instance_profile
  beanstalk_service_role         = each.value.beanstalk.iam.service_role

  load_balancer_public             = each.value.load_balancer.public
  load_balancer_log_bucket         = local.load_balancer_log_bucket
  load_balancer_log_prefix         = each.value.release.name
  load_balancer_ssl_certificate_id = each.value.load_balancer.ssl_certificate_id
  load_balancer_ssl_policy         = each.value.load_balancer.ssl_policy
  load_balancer_alias              = each.value.load_balancer.alias

  port_mappings = each.value.port_mappings

  extra_tags = each.value.extra_tags

  extra_settings = each.value.extra_settings
}
